...
import { faHandshake } from '@fortawesome/free-regular-svg-icons'; // for verified org
...
const post = { icon: faCircleInfo, label: "post", bg: "bg-primary" }    // Custom bg color
        ...
        <div className="px-5 leftsidecontainer-size">
            ...
            <ul className="list-unstyled d-flex flex-column gap-1">
                {menuItems.map((item, index) => (
                    <li
                        key={index}
                        className={`d-flex align-items-center gap-2 py-2 px-4  ${item.bg}`}
                    >
                ...
                <li className="d-flex align-items-center gap-2 py-2 px-4 rounded-5">
                    <img src={pic2} class="rounded-circle" alt="pic" style={{ width: '50px', height: '50px' }}></img>
